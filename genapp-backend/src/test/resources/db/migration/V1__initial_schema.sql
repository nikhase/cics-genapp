-- V1__initial_schema.sql (H2 version for testing)
-- H2-compatible schema for integration testing
-- Note: Uses standard SQL compatible with H2 in-memory database
-- Production and dev profiles use the PostgreSQL-specific version in src/main/resources
...
CREATE TABLE customer (
  customer_id UUID PRIMARY KEY DEFAULT RANDOM_UUID(),
...
CREATE TABLE feature_toggle (
  toggle_id UUID PRIMARY KEY DEFAULT RANDOM_UUID(),
...
-- - UUID fields use RANDOM_UUID() for automatic generation (H2 compatible)
...
-- - CLOB used instead of JSONB for H2 compatibility
